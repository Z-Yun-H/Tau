...
```bash
tau ask "列出所有 TODO 注释" --stream
```

`--stream` 会实时显示 provider 的思考过程（reasoning delta）：DeepSeek 的 `reasoning_content`、Anthropic 的 `thinking_delta`、Gemini 的 thought parts 都会以独立通道到达，与计划正文分开。计划本身的产出仍然要经过与缓冲模式完全一致的严格 JSON 校验与确定性审查——流式只改变"何时看到"，不改变"能否执行"。
