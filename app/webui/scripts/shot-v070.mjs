/**
 * WebUI v0.7.0 feature captures — real server + real client in headless
 * Chromium, mirroring scripts/shot-v060.mjs's approach (mock provider,
 * TAU_HOME sandbox, offline). Saves PNGs for the v0.7.0 features under
 * docs/screenshots/:
 *
 ...
 *   pnpm --filter @tau/webui shots:v070
 */
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "tau-webui-shots-v070-"));
process.env.TAU_HOME = path.join(tmp, "home");
fs.mkdirSync(process.env.TAU_HOME, { recursive: true });
fs.writeFileSync(
  path.join(tmp, "readme.md"),
  ["# demo workspace", "", "fixture workspace for the v0.7.0 shots", ""].join("\n"),
);
process.chdir(tmp);
