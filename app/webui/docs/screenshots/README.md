...
| `provider-setup.png`         | settings (v0.6.1): deepseek picked — the endpoint is looked up from the server catalog (`https://api.deepseek.com`), the key console is one link away, the key input waits      |
| `provider-setup-key.png`     | the pasted key masked (password dots) — the privacy default (v0.6.1)                                                                                                            |
| `provider-setup-reveal.png`  | explicit peek: the key in plaintext + "hide" — the toggle re-masks itself after 8s (v0.6.1)                                                                                     |
| `provider-setup-saved.png`   | after save: the server's `sk-***last4` mask, the `·key` chip on the provider, the input cleared, plaintext gone (v0.6.1)                                                        |
| `provider-setup-card.png`    | the provider setup section, element-cropped (v0.6.1)                                                                                                                            |
| `viewport-lock.png`          | the fixed-height shell (v0.6.1): a three-request thread scrolls inside the stream column, the third card clipped by the fold, the composer pinned — the page itself never grows |
| `model-picker.png`           | settings (v0.6.2): the provider section's catalog-backed model select (mock-chat / mock-reasoner served offline) and the honest "no thinking knobs" note for mock               |
| `thinking-controls.png`      | the provider section, element-cropped (v0.6.2): anthropic active — mode on/off + effort low/medium/high mini-pickers with "on (high)" applied via POST /api/config/thinking     |
| `thinking-controls-mock.png` | the same section with mock active (v0.6.2): providers without knobs render the honest note instead of dead controls                                                             |
...
pnpm --filter @tau/webui shots:v061   # v0.6.1 surfaces (provider setup/viewport lock)
pnpm --filter @tau/webui shots:v062   # v0.6.2 surfaces (model & thinking selection)
```
...
[`app/webui/scripts/shot-v061.mjs`](../../scripts/shot-v061.mjs) and
[`app/webui/scripts/shot-v062.mjs`](../../scripts/shot-v062.mjs).
...
harnesses is fully end-to-end: `shot-v061.mjs` needs no interception at all — its provider save goes
...
never scrolls (`pageScrollHeight == innerHeight`) while the stream column overflows. `shot-v062.mjs`
is likewise interception-free: the model choice and the thinking writes go through the actual `POST
/api/config/model` / `POST /api/config/thinking` endpoints into the sandbox.
```
